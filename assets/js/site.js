import { siteContent } from "./site-data.js?v=20260828a";
...
function createSocialLinks(position) {
  const group = document.createElement("div");
  group.className = `social-links social-links-${position}`;
  group.setAttribute("aria-label", "Atho community links");

  siteContent.socialLinks.forEach((item) => {
    const link = document.createElement("a");
    ...
    svg.setAttribute("aria-hidden", "true");
    const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
    path.setAttribute("d", brandIconPaths[item.icon]);
    svg.appendChild(path);
...
function renderSocialLinks() {
  const headerTarget = document.querySelector(".nav-actions, .docs-header-actions");
  if (headerTarget instanceof HTMLElement && !headerTarget.querySelector(".social-links-header")) {
    headerTarget.prepend(createSocialLinks("header"));
  }

  const footerTarget = document.querySelector(".footer-brand");
  if (footerTarget instanceof HTMLElement && !footerTarget.querySelector(".social-links-footer")) {
    footerTarget.appendChild(createSocialLinks("footer"));
  }
}
...
function initSupplyCounter() {
  const counter = document.querySelector("[data-supply-counter]");
  ...
  const render = (value) => {
    counter.textContent = formatter.format(Math.min(target, Math.max(0, Math.round(value))));
  };
