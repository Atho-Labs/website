...
- **Emissions (current):** five fixed pre-tail eras (`10 -> 5 -> 2.5 -> 1.25 -> 0.625`, each `1,314,000` blocks), then transition reward `0.3125 ATHO/block` until exact pre-tail supply reaches `30,000,000 ATHO`, then perpetual tail `0.25 ATHO/block`. Tail starts at height `21,102,000` (~year `80.3`). Tail issuance is `65,700 ATHO/year`.
- **Fees/Burn (current):** fee floor `200 atoms/vB` (`2.0e-7 ATHO/vB`), minimum tx fee `187,500 atoms` (`0.000187500 ATHO`), dust `5 atoms`. Post-tail target split is `100% burn / 0% miner fee share`, clipped by a hard supply floor at `21,000,000 ATHO`.
- **Inflation/Deflation regime:** protocol net post-tail change is utilization-dependent; under current constants the deflation threshold is `>50%` sustained block-byte utilization.
