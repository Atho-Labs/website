# Atho Documentation Index

Date: 2026-04-04

This is the canonical documentation map for current development builds.

## Start Here
1. [quickstart.md](quickstart.md) - install, build native binaries, run node and GUI.
2. [Binaries.md](Binaries.md) - binary layout, hash pinning, and strict runtime mode.
3. [Troubleshooting.md](Troubleshooting.md) - common runtime/build failures and fixes.

## Core Protocol
- [WhitePaper.md](WhitePaper.md)
- [Consensus.md](Consensus.md)
- [Tx.md](Tx.md)
- [PlatinumShield_Privacy.md](PlatinumShield_Privacy.md)
- [Sigwit.md](Sigwit.md)
- [Sha3-384.md](Sha3-384.md)
- [Falcon512.md](Falcon512.md)
- [Emissions.md](Emissions.md)
- [Inflation_Deflationary.md](Inflation_Deflationary.md)
- [Emissions Modeling/Entire_Overview.md](Emissions Modeling/Entire_Overview.md) (contains legacy-model body plus active-policy correction header)

For BPoW, miner bonding, and wallet staking:
- start with [WhitePaper.md](WhitePaper.md) section 5,
- then read [Consensus.md](Consensus.md) sections 2, 2.1, 2.2, and 6.1,
- then [Tx.md](Tx.md) section 9.1 for role-aware lockup transaction behavior.

## Runtime and Operations
- [Network_Stack.md](Network_Stack.md)
- [ApiAuth.md](ApiAuth.md)
- [LMDB.md](LMDB.md)
- [KeyManager.md](KeyManager.md)
- [Base56.md](Base56.md)
- [GPU_Miner.md](GPU_Miner.md)
- [Docker.md](Docker.md)
- [dockertest.md](dockertest.md)
- [Packaging.md](Packaging.md)
- [Node_Stop.md](Node_Stop.md)

## Developer References
- [ONBOARDING.md](ONBOARDING.md)
- [SRC_FILE_MAP.md](SRC_FILE_MAP.md)
- [gui.md](gui.md)
